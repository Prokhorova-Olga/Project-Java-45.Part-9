...
public class BasketService {
    ...
    public void addProductById(UUID id) {
        ...
        if (productOptional.isEmpty()) {
            throw new NoSuchProductException("Продукт с таким id " + id + " не найден");
        }
    ...
    public UserBasket getUserBasket() {
        ...
        List<BasketItem> basketItems = basketProducts.entrySet().stream().map(entry -> {
            ...
            Product product = storageService.getProductById(productId).orElseThrow(() -> new NoSuchProductException("Продукт с таким id " + productId + " не найден"));
